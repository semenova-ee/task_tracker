...
def get_free_empl():
    ...
    min_task_count = Employee.objects.annotate(task_count=Count('tasks')).aggregate(min_task_count=Min('task_count'))['min_task_count']

    # Step 2: Get the employees with the minimum number of tasks
    least_busy_employees = Employee.objects.annotate(task_count=Count('tasks')).filter(task_count=min_task_count)
